...
flagged as (
    select
        ...
        case when campaign_id is null then 'Missing campaign_id' else null end as err_id,
        case when name is null then 'Missing name' else null end as err_name,
        case when budget is null then 'Missing budget'
             when budget <= 0 then 'budget <= 0' else null end as err_budget,
